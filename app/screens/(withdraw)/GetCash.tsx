...
const GetCash = () => {
    ...
    //...
    const {
        data: withdrawalBanksData,
        isLoading: isWithdrawalBanksLoading,
        isError,
        refetch,
    } = useQuery({
        queryKey: ["getWithdrawalBankAccounts", userId],
        queryFn: async () => await getWithdrawalBankAccounts(),
    ...
    return (
        <CustomSafeArea topColor="#ffffff" bgColor="#ffffff">
            <View style={{ paddingHorizontal: size.getHeightSize(24) }}>
                <GenericHeader title={"Get cash"} currency="ngn" />
            </View>
            {isWithdrawalBanksLoading || isError ? (
                <PageLoader />
            ) : (
                <ScrollView
                ...
                >
                    {withdrawalBanksData.data.length == 0 ? (
                        <View
                            style={{
                        ...
                        <View
                            style={{ marginVertical: size.getHeightSize(24) }}
                        >
                            <View
                                style={{
                                    flexDirection: "row",
                        ...
                        <View>
                            ...
                            >
                                <Text style={styles.label}>
                                    Amount to to withdraw
                                </Text>
                                <Text style={styles.labelBal}>
                                    Balance: NGN{" "}
                                    {formatCurrency({
                                        value: withdrawalBanksData.user
                                            .ngnBalance,
                                        currency: "ngn",
                                    })}
                                </Text>
